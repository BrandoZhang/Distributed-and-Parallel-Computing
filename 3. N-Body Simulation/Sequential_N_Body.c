/* Sequential N-Body simulation with Barnes-Hut */
...
int main(int argc, char const *argv[])
{
	...
	if (argc < 6)
	{
		printf("Hint: Execute with %s X_RESN Y_RESN N MAX_STEP X11_ENABLE\n", argv[0]);
		exit(EXIT_FAILURE);
	}
	...
	FILE *log_file_p;
	char file_name[100];
	sprintf(file_name, "Sequential_of_%d-Body_in_%dx%d_img_to_%d_steps.txt", N, X_RESN, Y_RESN, MAX_STEP);
